"""Momentum strategy (EMA crossover + ADX trend filter)."""
...
class MomentumParams:
    ...
    @staticmethod
    def from_dict(data: dict) -> "MomentumParams":
...
class MomentumStrategy(TradingStrategy):
    ...
    def on_bar(self, bar: PriceBar) -> list[StrategyDecision]:
        ...
        fast = self._ema(series.closes, self.params.ema_fast)
        slow = self._ema(series.closes, self.params.ema_slow)
        prev_fast = self._ema(series.closes[:-1], self.params.ema_fast) if len(series.closes) > 1 else None
        prev_slow = self._ema(series.closes[:-1], self.params.ema_slow) if len(series.closes) > 1 else None
...
def build_momentum_from_config(
    ...
    params = MomentumParams.from_dict(parameters)
    ...
    return MomentumStrategy(params, sizer, timezone, initial_balance)
